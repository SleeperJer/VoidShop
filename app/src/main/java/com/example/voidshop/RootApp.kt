...
fun RootApp(
    ...
    // Pantalla de detalle (si hay producto seleccionado)
    selectedProductId?.let { pid ->
        val p = products.firstOrNull { it.id == pid }
        if (p != null) {
            val det = ProductDetailsRepo.get(p)
            val cartCount = cartViewModel.count() // badge en detalle
            ProductDetailScreen(
                product = p,
                details = det,
                cartCount = cartCount,
    ...
    Scaffold(
        ...
        Column(
            ...
            // Buscador + hamburguesa de filtros en trailingIcon
            OutlinedTextField(
                value = query,
            ...
            LazyColumn(
                ...
                items(filtered) { p: Product ->
                    ProductCard(
                        ...
                        onOpenDetails = { selectedProductId = p.id } // ← abre detalles
                    )
                }
                if (filtered.isEmpty()) {
                    item {
